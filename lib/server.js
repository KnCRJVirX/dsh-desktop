...
const { applySystemProxy } = require('./proxy');
...
async function startServer(opts = {}) {
  ...
  // `--no-open` (DSH 0.1.2-rc.1+): the Web UI is shown in the Electron window,
  // not in the system default browser.
  const args = [bin, 'web', '--host', host, '--port', String(port), '--no-open'];

  const env = { ...process.env, ...node.extraEnv };
  if (dshHome) env.DSH_HOME = dshHome;

  // DSH reads proxy configuration only from the environment, never from the OS
  // proxy settings, so forward the machine's proxy here — otherwise outbound
  // tools (web_fetch, web search, LLM, HTTP MCP) connect directly and fail on
  // networks that require a proxy.
  const proxy = applySystemProxy(env);
  if (proxy.applied) {
    console.log(`[dsh-desktop] forwarding system proxy ${String(proxy.proxy)} to DSH`);
  } else if (proxy.reason === 'explicit') {
    console.log('[dsh-desktop] proxy environment already set; leaving it untouched');
  }
